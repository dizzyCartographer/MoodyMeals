...
- [TF-2] **[BLOCKED(app record)]** (30m) First TestFlight upload — run `scripts/testflight.sh --upload`, confirm processing, internal-tester install. ~~PLA~~ accepted 2026-07-12 (archive + .ipa export verified end-to-end same day; bundle IDs + App Group registered). Remaining: Maria creates the App Store Connect app record — name **"Moody Meals"** per D-49, bundle ID `com.mariayarley.Moody` from the dropdown, any SKU. **AC:** build visible in TestFlight, installable on Maria's phone.
