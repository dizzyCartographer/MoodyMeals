...
- [TF-2] **[BLOCKED(PLA + app record)]** (30m) First TestFlight upload — run `scripts/testflight.sh --upload`, confirm processing, internal-tester install. Blocked on: Maria accepts the updated Apple Program License Agreement (developer.apple.com, ~1 min) and creates the App Store Connect app record (name decision in the 2026-07-12 digest). **AC:** build visible in TestFlight, installable on Maria's phone.
