...
- [M4-0] **[BLOCKED(D-42)]** (2×90m — split candidate M4-0a/M4-0b) Generalize food rules + D-44 GF risk-class model (added 2026-07-09; amended twice per Ria) — **(a)** replace `DietaryRequirement`/`FoodNeedGoal` with per-member `FoodRule` {never/limit/boost, tags, frequency knob}; seed: Chuck limit{redMeat, pork} ≤1×/wk; Ria boost{iron, fiber, antiInflammatory}; Chad boost{calorieDense}; Elsie NO rule (D-35 objective + scores + staples carry the end goal); rule-filterable ingredient views. **(b)** replace tri-state `isGlutenFreeVerified` with D-44: ingredient risk class {wholeFood | glutenCarrier | packaged}, per-ingredient preferred brand, per-recipe standard modifications, HOUSEHOLD standard substitutions with bulk-apply checklist (D-44 refinement — per-recipe subs override household defaults), recipe-level "substitutions may be required" indicator (calm styling per design law #4, never alarm); shopping list carries sub/preferred brands (RT-6 pipe). **AC:** §1 REWRITTEN to D-44 invariants and submitted for Ria's sign-off before old tests are deleted (never silent); whole-food-never-prompts; carrier-without-sub-flagged-and-never-Caddie-ready; sub-or-brand-documented ⇒ safe with indicator fully cleared; bulk-apply clears N ticked recipes in one action while unticked keep the flag; per-recipe-sub-overrides-household-default; add-a-rule-at-runtime (new restriction honored with zero code change). NEEDS-VISUAL-REVIEW (rule/brand/sub/bulk UI).
