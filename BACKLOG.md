...
- [M0-5] **[BLOCKED(D-34)]** (60m) Core models, shopping/inventory/etc — `ShoppingRun/Item`, `Snack`, `PurchaseRecord`, `InventoryItem`, `WasteEvent`, `CheckIn`, `WeeklyReflection`, `FridgeSpec`. **AC:** persistence tests for each. → needs Ria's one-word confirm on adding `staple` to ItemSource (D-34); D-33 fields (kind/useBy) approved ✓.
