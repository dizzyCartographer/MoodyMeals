...
- [M3-2] (90m) Brain-dump → recipe (`createRecipe`) — parse messy text into Loose/Precise. *(D-44 re-scope: the same capture call runs the GF assessment — any gluten? which lines? easily substitutable in context? → band + suggested subs stored on the recipe; rubric pins: homemade bread→unsafe, mac-n-cheese→"which gf pasta?", tbsp flour→calm sub request; whole foods never flagged.)* **AC:** NL-3, HC-7 as re-scoped by D-44, rubric example pins as eval fixtures. PROMPT-REVIEW.
