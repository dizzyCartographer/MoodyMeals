...
- [M4-0] **[BLOCKED(D-42)]** (90m+) Generalize food rules (added 2026-07-09, Ria-proposed; amended same day) — replace `DietaryRequirement`/`FoodNeedGoal` AND the tri-state `isGlutenFreeVerified` with per-member `FoodRule` {never/limit/boost/require, tags, frequency knob}; allergen + certification tags on ingredients and meals/recipes (meal-level gf tag covers checked takeout); migrate seed (Caddie require{gf}; Chuck limit{redMeat, pork}; Ria boost{iron, fiber, antiInflammatory}; Chad boost{calorieDense}; Elsie boost{protein, veg, starch} @ ~nightly — plate rule folded in, staples untouched); rule-filterable ingredient views. **AC:** §1 suite behavior-identical and green (HC-1..7, SF-1..3 — untagged = not-for-Caddie); add-a-rule-at-runtime test (new restriction honored with zero code change). NEEDS-VISUAL-REVIEW (rule/tag UI).
