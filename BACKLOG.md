...
- [M3-2] (90m) Brain-dump → recipe (`createRecipe`) — parse messy text into Loose/Precise. *(D-44 re-scope: "gluten defaults unverified" replaced by capture-time carrier flagging — Claude flags likely gluten carriers ("beer-battered fish" → beer batter) and prompts to record the standard sub; whole foods never prompt.)* **AC:** NL-3, HC-7 as re-scoped by D-44. PROMPT-REVIEW.
...
- [M4-0] **[BLOCKED(D-42)]** (2×90m — split candidate M4-0a/M4-0b) Generalize food rules + D-44 GF risk-class model (added 2026-07-09; amended twice per Ria) — **(a)** replace `DietaryRequirement`/`FoodNeedGoal` with per-member `FoodRule` {never/limit/boost, tags, frequency knob}; seed: Chuck limit{redMeat, pork} ≤1×/wk; Ria boost{iron, fiber, antiInflammatory}; Chad boost{calorieDense}; Elsie NO rule (D-35 objective + scores + staples carry the end goal); rule-filterable ingredient views. **(b)** replace tri-state `isGlutenFreeVerified` with D-44: ingredient risk class {wholeFood | glutenCarrier | packaged}, carrier substitution profiles {dropIn | quantitySensitive | nonSubstitutable}, SCORED severity {awaitingSubstitution | unsafe} from count-share + normalized-volume thresholds (TuningConfig: carrierStructuralCountShare=0.25, carrierStructuralVolumeCups=1 — documented defaults; Loose/amount-less recipes fall back to count share, ambiguity → calmer band), per-ingredient preferred brand, per-recipe standard modifications, HOUSEHOLD standard substitutions with bulk-apply checklist (full preview names every recipe + exact line changing; per-recipe subs override household defaults), "awaiting substitution" indicator calm per design law #4, unsafe tier nuclear (aligns w/ HC-9 flour line); shopping list carries sub/preferred brands (RT-6 pipe). **AC:** §1 REWRITTEN to D-44 invariants and submitted for Ria's sign-off before old tests are deleted (never silent); whole-food-never-prompts; BOTH bands block Caddie auto-scheduling until resolved (tone differs, scheduler doesn't); D-44 example pins: homemade-bread→unsafe, mac-n-cheese→calm question, tbsp-flour-in-20→calm question; sub-or-brand-documented ⇒ safe with indicator fully cleared; bulk-apply clears N ticked recipes in one action while unticked keep the flag; add-a-rule-at-runtime (new restriction honored with zero code change). NEEDS-VISUAL-REVIEW (rule/brand/sub/bulk UI).
