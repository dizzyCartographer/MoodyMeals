...
A parallel build surfaced 46 user-ratified product decisions (`UNIFICATION_PLAN.md` has the full story; D-42…D-46 landed 2026-07-09 in Ria's live cloud session — read them in DECISIONS.md before designing food-safety or library surfaces). Her goal sentence, now canon (D-46): *"It should be crazy easy for me to see who loves this meal, who this meal serves. That's the goal: to feed my family more intentionally and more enjoyably."* The ones that change mockups:
...
- **Safety badges are tri-state, per-attendee, derived (D-35, D-38, HC-3/5).** *⚠️ SUPERSEDED by D-44 (2026-07-09): the tri-state is the shipped interim model only. Go-forward bands are safe / awaiting-substitution / unsafe; awaiting-substitution is frictionlessly schedulable (calm indicator, never a warning); the HC-5 deliberate-weight confirm attaches ONLY to the unsafe band. Per-attendee derivation and Caddie-away states survive unchanged.* Never hardcode member names in designs as if always-present; badges reflect who's ATTENDING (Caddie away unlocks gluten — design that state). No red anywhere (law 4) — yellow is the ceiling; but the unsafe confirm carries real gravity. This is the one place the kit's candy language must carry medical seriousness.
...
- **Meal library (Wave-1 item 4) has a working behavioral spec** in the parallel build's plain UI (full CRUD field inventory, loose/precise recipes) — mock the *design*; the fields and flows are already decided. *⚠️ The tri-state GF badge there is superseded by D-44 (bands) and D-46 (per-person ❤️ and ✅/❌ chips on library rows + "serves X" / "loved by X" filters) — read both in DECISIONS.md before mocking the library.*
...
7. **Settings root + household & safety editor.** No settings surface exists at all. The household editor is the serious one — *superseded in part by D-42/D-44/D-45 (2026-07-09): its unit is the structured **FoodRule** record ({member, never/limit/boost, subject, reason, frequency window} — identical rule shapes get identical UI), Caddie's celiac protection is the D-44 band model, and **Elsie has NO rule** (her lifeline lives under always-stocked; dinners are carried by liking scores + the fairness floor). Chuck's limit{red meat, pork} and Ria/Chad's boosts need homes here too.* Cook nights stay — onboarding promises "adjust anytime — the week re-solves around safety." Give safety edits deliberate weight (this is medical-adjacent data, not preferences). Settings root also hangs: cast management, palette editor, always-stocked editor + fallback-meal picker, AM breakfast defaults, notifications, and the privacy surface (item 10).
...
9. **Onboarding steps 4–5 + resume.** The stressor interview and per-member meal swipe-rating (~3 min each, skippable) are named in your README but unmocked — swipe-rating is where taste data comes from *(per D-42 there is no "Elsie's safe list" rule anymore — her deck rates like everyone's; likings + the fairness floor carry her dinners)*. Onboarding is long; design the **interrupted/resume** state (she *will* background the app mid-flow).
